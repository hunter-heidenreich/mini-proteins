...
The Python scripts are invoked by the shell scripts and also read `ID` from the environment. **Environment is managed with `uv`** (`pyproject.toml` + committed `uv.lock`): `uv sync` installs the core deps (numpy/matplotlib, enough for the GROMACS analysis); `uv sync --extra ml` adds the OpenMM/benchmark track (openmm, mdtraj, deeptime). The pipeline scripts call `uv run` internally, so `ID=ala sh scripts/run.sh` works as-is; benchmark entry points run as `uv run --extra ml python -m benchmark.<...>`.
...
- **git:** code, configs, input PDBs, docs, env lock (`uv.lock`), and **small certified reports** committed under `results/` (the `out/` tree is gitignored, so `scripts/pull_results.sh` copies `validation.*`/`curation.*`/figures into `results/` for committing — run it before stopping the pod).
